...
Arguments:
    ...
    phi_max (float): phi_max*pi is the max phase delay of the pattern reached
    ...
    center_spot (int): spot number of a spot to hide
    darken_cspot (bool): if True hides spot `dark_spot`
    lw (int): line-width of the steering pattern
    vmax (int, 0..255): max level for the steering pattern
    ph_wrapping (bool): if True wrap the phase above 2*pi
